...
const readBody = async (req: import("node:http").IncomingMessage) => { let data = ""; for await (const chunk of req) data += chunk; return data ? JSON.parse(data) : {}; };

const server = createServer(async (req, res) => {
  const url = new URL(req.url ?? "/", "http://localhost");
  if (url.pathname === "/api/event-types" && req.method === "GET") return json(res, 200, eventTypes);
  if (url.pathname === "/api/slots" && req.method === "GET") return json(res, 200, slots.filter((s) => !s.bookedBy));
  if (url.pathname === "/api/bookings" && req.method === "POST") {
    const body = await readBody(req); const slot = slots.find((s) => s.id === body.slotId);
    if (!slot || slot.bookedBy) return json(res, 409, { error: "Slot is already booked or missing" });
